...
export type RuleInfo = { id: string; name: string; description: string; tag: string; source: string };
...
export default function App({ rules }: { rules: RuleInfo[] }) {
  ...
  return <div className="shell">
    ...
    <main>
      <section className="intro"><div><p className="eyebrow">YOUR CODE. YOUR RULES.</p><h1>코드를 넣고, 규칙을 확인하세요<span>.</span></h1><p className="subtitle">TypeScript 코드에 Rust 규칙을 적용하고, 놓치기 쉬운 패턴을 찾아보세요.</p></div><span className="version">v0.1 <span>EXPERIMENTAL</span></span></section>
    ...
    </main><footer><span>ai-lint <span className="footer-muted">/ 코드의 문맥을 읽는 린터</span></span><span>Rust + Oxc <span className="separator">·</span> Rust rules</span></footer>
  </div>;
}
